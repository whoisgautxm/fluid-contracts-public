...
contract VaultT1Admin is Variables, Events, Error {
    ...
    modifier _updateExchangePrice() {
        IVaultT1(address(this)).updateExchangePricesOnStorage();
        _;
    }
    ...
    function rescueFunds(address token_) external _verifyCaller {
        if (token_ == NATIVE_TOKEN) {
            Address.sendValue(payable(IVaultT1(address(this)).LIQUIDITY()), address(this).balance);
        } else {
            SafeERC20.safeTransfer(
                IERC20(token_),
                IVaultT1(address(this)).LIQUIDITY(),
    ...
    function absorbDustDebt(uint[] memory nftIds_) public _verifyCaller {
        ...
        for (uint i = 0; i < nftIds_.length; ) {
            ...
            if (((tickData_ & 1) == 1) || (((tickData_ >> 1) & X24) > tickId_)) {
                // User got liquidated
                (, posDebt_, , , ) = IVaultT1(address(this)).fetchLatestPosition(
                    posTick_,
                    tickId_,
                    posDebt_,
                    tickData_
