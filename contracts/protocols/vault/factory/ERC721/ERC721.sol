...
abstract contract ERC721 is Error {
    ...
    */
    ...
    function transferFrom(address from_, address to_, uint256 id_) public virtual {
        ...
        if (to_ == address(0)) revert FluidVaultError(ErrorTypes.ERC721__InvalidParams);
        ...
        _transfer(from_, to_, id_, (tokenConfig_ >> 192) & 0xFFFFFFFF);
    ...
    function _transfer(address from_, address to_, uint256 id_, uint256 vaultId_) internal {
        if (from_ == address(0)) {
            ...
            vaultId_ = _remove(from_, id_);
            _add(to_, id_, vaultId_);
        }
    }

    function _add(address user_, uint256 id_, uint256 vaultId_) internal {
    ...
    function _remove(address user_, uint256 id_) internal returns (uint256 vaultId_) {
        uint256 temp_ = _tokenConfig[id_];

        // fetching `id_` details and deleting it.
        uint256 tokenIndex_ = (temp_ >> 160) & type(uint32).max;
        vaultId_ = (temp_ >> 192) & type(uint32).max;
        delete _tokenConfig[id_];
        ...
        uint256 lastTokenId_ = id_;
        {
            unchecked {
                ...
                if (lastTokenIndex_ != tokenIndex_) {
                    ...
                    lastTokenId_ = uint256((temp_ >> lastTokenBitShift_) & type(uint32).max);
        ...
        _tokenConfig[lastTokenId_] =
            (_tokenConfig[lastTokenId_] & ~(uint256(type(uint192).max))) |
            uint160(user_) |
            (tokenIndex_ << 160) |
            (vaultId_ << 192);
    }
