...
abstract contract Constants {
    ...
    /// @dev max allowed reward rate is 25%
    uint256 internal constant MAX_REWARDS_RATE = 25 * 1e12; // 25%;

    /// @dev address of the Liquidity contract.
    ILiquidity internal immutable LIQUIDITY;

    /// @dev address of the Lending factory contract.
    ILendingFactory internal immutable LENDING_FACTORY;

    /// @dev address of the underlying asset contract.
    IERC20 internal immutable ASSET;

    /// @dev number of decimals for the iToken, same as ASSET
    ...
    constructor(ILiquidity liquidity_, ILendingFactory lendingFactory_, IERC20 asset_) {
...
abstract contract Variables is ERC20, ERC20Permit, Error, Constants, IIToken {
    /// @dev prefix for token name. iToken will append the underlying asset name
    string private constant TOKEN_NAME_PREFIX = "Fluid Interest ";
    /// @dev prefix for token symbol. iToken will append the underlying asset symbol
    string private constant TOKEN_SYMBOL_PREFIX = "fi";
    ...
    ILendingRewardsRateModel internal _rewardsRateModel;
    ...
    /// @dev exchange price between iToken and the underlying assset (with rewards)
