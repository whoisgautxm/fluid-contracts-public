...
import { IIToken, IITokenAdmin } from "../interfaces/iIToken.sol";
...
abstract contract ReentrancyGuard is Error, Variables {
    ...
    function _checkReentrancy() internal {
        ...
        if (_status != REENTRANCY_NOT_ENTERED) {
            revert FluidLendingError(ErrorTypes.iToken__Reentrancy);
        }
...
/// @dev internal methods for iToken contracts
abstract contract iTokenCore is Error, IERC4626, IIToken, Variables, Events, ReentrancyGuard {
    ...
    function _calculateNewTokenExchangePrice(
        ...
        if (newLiquidityExchangePrice_ < oldLiquidityExchangePrice_) {
            ...
            revert FluidLendingError(ErrorTypes.iToken__LiquidityExchangePriceUnexpected);
        }
    ...
    function _updateRates(
        ...
        if (_rewardsActive || forceUpdateStorage_) {
            ...
            if (_tokenExchangePrice > type(uint64).max) {
                revert FluidLendingError(ErrorTypes.iToken__ExchangePriceOverflow);
            }
    ...
    function _executeDeposit(
        ...
        /*
            The check for `sharesMinted_` being 0 has been removed for the following reasons:

            1. Such a scenario is extremely unlikely: In practical scenarios, users are not expected to deposit 
               insignificant amounts that result in zero shares being minted.
            2. The protocol remains robust: Even in the edge case where someone does deposit a minuscule amount, 
               it doesn't negatively affect the protocol.
            3. There's a safer alternative: For users concerned about precision, there's an alternative method 
               that accepts an explicit `minAmountOut` parameter. As recommended in the natspec, they can set a 
               `minAmountOut` of 1, ensuring they get at least one share or the transaction reverts.
            4. If `assets_` was 0, an error 'UserModule__OperateAmountsZero' would already have been thrown during 
               the `operate` function, ensuring the contract never reaches this point with a zero `assets_` value.

            By removing the check, we simplify the contract and reduce the gas cost slightly.

            Note: It's crucial to keep the user's perspective in mind. 
            If ever the contract's behavior changes or new patterns of use emerge, this decision might need reevaluation.
        */
...
/// @notice iToken view methods. Implements view methods for ERC4626 compatibility
abstract contract iTokenViews is iTokenCore {
    using FixedPointMathLib for uint256;
    ...
    function maxDeposit(address) public view virtual override returns (uint256) {
        // read total supplyRawInterest_ for the token at Liquidity and convert from BigMath
        uint256 supplyRawInterest_ = LIQUIDITY.readFromStorage(LIQUIDITY_TOTAL_AMOUNTS_SLOT) & LiquidityCalcs.X64;
        supplyRawInterest_ =
            (supplyRawInterest_ >> LiquidityCalcs.DEFAULT_EXPONENT_SIZE) <<
            (supplyRawInterest_ & LiquidityCalcs.DEFAULT_EXPONENT_MASK);

        unchecked {
            // type(int128).max is the maximum interactable amount at Liquidity. But also total token amounts
            // must not overflow type(int128).max, so max depositable is type(int128).max - totalSupplyRaw.
            uint256 depositableRaw_ = uint256(uint128(type(int128).max)) - supplyRawInterest_;
            // normalize from raw
            return (depositableRaw_ * _getLiquidityExchangePrice()) / EXCHANGE_PRICES_PRECISION;
        }
    }
    ...
    function minDeposit() public view returns (uint256) {
        return previewMint(1); // rounds up
    }
}

/// @notice iToken admin related methods. iToken admins are Lending Factory auths. Possible actions are
/// updating rewards, funding rewards, and rescuing any stuck funds (iToken contract itself never holds any funds).
abstract contract iTokenAdmin is iTokenCore, iTokenViews {
    ...
    function _checkIsLendingFactoryAuth() internal view {
        if (!LENDING_FACTORY.isAuth(msg.sender)) {
            revert FluidLendingError(ErrorTypes.iToken__Unauthorized);
        }
    }

    /// @inheritdoc IITokenAdmin
    ...
    function rebalance() external payable virtual nonReentrant returns (uint256 assets_) {
        if (msg.sender != _rebalancer) {
            revert FluidLendingError(ErrorTypes.iToken__NotRebalancer);
        }
        if (msg.value > 0) {
            revert FluidLendingError(ErrorTypes.iToken__NotNativeUnderlying);
        }
        ...
        // no shares are minted when funding iToken contract for rewards
...
/// @notice iToken public executable actions: deposit, mint, mithdraw and redeem.
...
abstract contract iTokenActions is iTokenCore, iTokenViews {
    ...
    function _revertIfAboveMaxAmount(uint256 amount_, uint256 maxAmount_) internal pure {
        if (amount_ > maxAmount_) {
            revert FluidLendingError(ErrorTypes.iToken__MaxAmount);
        }
    }
    ...
    /// @notice same as {iToken-deposit} but with an additional setting for minimum output amount.
    ...
    /// @notice same as {iToken-mint} but with an additional setting for maximum assets input amount.
    ...
    /// @inheritdoc IERC4626
    /// @notice If `assets_` equals uint256.max then the whole iToken balance of `owner_` is withdrawn. This does not
    ...
    /// @notice same as {iToken-withdraw} but with an additional setting for maximum shares burned.
    ...
    /// @notice same as {iToken-redeem} but with an additional setting for minimum output amount.
...
/// @notice iTokens support EIP-2612 permit approvals via signature so this contract implements
/// withdrawals (withdraw / redeem) with signature used for approval of the iToken shares.
abstract contract iTokenEIP2612Withdrawals is iTokenActions {
    ...
    /// @notice withdraw amount of `assets_` with ERC-2612 permit signature for iToken approval.
    ...
    function withdrawWithSignature(
        ...
        if (msg.sender == owner_) {
            ...
            revert FluidLendingError(ErrorTypes.iToken__PermitFromOwnerCall);
        }
    ...
    /// @notice redeem amount of `shares_` with ERC-2612 permit signature for iToken approval.
    /// `owner_` signs ERC-2612 permit `signature_` to give allowance of iTokens to `msg.sender`.
...
/// @notice implements iTokens support for deposit / mint via EIP-2612 permit.
/// @dev methods revert if underlying asset does not support EIP-2612.
abstract contract iTokenEIP2612Deposits is iTokenActions {
...
/// @notice implements iTokens support for deposit / mint via Permit2 signature.
abstract contract iTokenPermit2Deposits is iTokenActions {
    /// @inheritdoc IIToken
...
/// @title Fluid iToken (Lending with interest)
/// @notice iToken is a token that can be used to supply liquidity to the Fluid Liquidity pool and earn interest for doing so.
/// The iToken is backed by the underlying balance and can be redeemed for the underlying token at any time.
/// The interest is earned via Fluid Liquidity, e.g. because borrowers pay a borrow rate on it. In addition, iTokens may also
/// have active rewards going on that count towards the earned yield for iToken holders.
/// @dev The iToken implements the ERC20 and ERC4626 standard, which means it can be transferred, minted and burned.
/// The iToken supports EIP-2612 permit approvals via signature.
/// The iToken implements withdrawals via EIP-2612 permits and deposits with Permit2 or EIP-2612 (if underlying supports it) signatures.
/// iTokens are not upgradeable.
/// @dev For view methods / accessing data, use the "LendingResolver" periphery contract.
contract iToken is iTokenAdmin, iTokenActions, iTokenEIP2612Withdrawals, iTokenPermit2Deposits, iTokenEIP2612Deposits {
    ...
    function liquidityCallback(address token_, uint256 amount_, bytes calldata data_) external virtual override {
        ...
        if (data_.length == 32) {
            ...
            if (!isPermit2_) {
                ...
                revert FluidLendingError(ErrorTypes.iToken__InvalidParams);
            }
