...
contract VaultResolver is Helpers {
    ...
    function getVaultVariablesRaw(address vault_) public view returns (uint) {
        return IVaultT1(vault_).readFromStorage(normalSlot(0));
    }

    function getVaultVariables2Raw(address vault_) public view returns (uint) {
        return IVaultT1(vault_).readFromStorage(normalSlot(1));
    }

    function getAbsorbedLiquidityRaw(address vault_) public view returns (uint) {
        return IVaultT1(vault_).readFromStorage(normalSlot(2));
    }

    function getPositionDataRaw(address vault_, uint positionId_) public view returns (uint) {
        return IVaultT1(vault_).readFromStorage(calculateStorageSlotUintMapping(3, positionId_));
    }
    ...
    function getTickHasDebtRaw(address vault_, int key_) public view returns (uint) {
        return IVaultT1(vault_).readFromStorage(calculateStorageSlotIntMapping(4, key_));
    }

    function getTickDataRaw(address vault_, int tick_) public view returns (uint) {
        return IVaultT1(vault_).readFromStorage(calculateStorageSlotIntMapping(5, tick_));
    }

    // TODO: Verify below
    // id_ = (realId_ / 3) + 1
    function getTickIdDataRaw(address vault_, int tick_, uint id_) public view returns (uint) {
        return IVaultT1(vault_).readFromStorage(calculateDoubleIntUintMapping(6, tick_, id_));
    }

    function getBranchDataRaw(address vault_, uint branch_) public view returns (uint) {
        return IVaultT1(vault_).readFromStorage(calculateStorageSlotUintMapping(7, branch_));
    }

    function getRateRaw(address vault_) public view returns (uint) {
        return IVaultT1(vault_).readFromStorage(normalSlot(8));
    }

    function getRebalancer(address vault_) public view returns (address) {
        return address(uint160(IVaultT1(vault_).readFromStorage(normalSlot(9))));
    }
    ...
    function getVaultConstants(address vault_) internal view returns (IVaultT1.ConstantViews memory constants_) {
        constants_ = IVaultT1(vault_).constantsView();
    }
    ...
    function getExchangePricesAndRates(
        ...
        ) = IVaultT1(vault_).updateExchangePrices(getVaultVariables2Raw(vault_));
    ...
    function getTotalSupplyAndBorrow(
        ...
        IVaultT1.ConstantViews memory constantsVariables_
    ...
    function positionByNftId(
        ...
        if (!userPosition_.isSupplyPosition) {
            ...
            if (((tickData_ & 1) == 1) || (((tickData_ >> 1) & X24) > userPosition_.tickId)) {
                ...
                (userPosition_.tick, userPosition_.borrow, userPosition_.supply, , ) = IVaultT1(vault_)
    ...
    function getVaultLiquidation(
        ...
        IVaultT1.ConstantViews memory constants_ = getVaultConstants(vault_);
        ...
        try IVaultT1(vault_).liquidate(tokenInAmt_, 0, 0x000000000000000000000000000000000000dEaD, false) {
            ...
            if (lowLevelData_.length >= 68) {
                ...
                if (errorSelector_ == IVaultT1.FluidLiquidateResult.selector) {
                    assembly {
        ...
        // running with absorb
        try IVaultT1(vault_).liquidate(tokenInAmt_, 0, 0x000000000000000000000000000000000000dEaD, true) {
            // Handle successful execution
        } catch Error(string memory reason) {
            // Handle generic errors with a reason
        } catch (bytes memory lowLevelData_) {
